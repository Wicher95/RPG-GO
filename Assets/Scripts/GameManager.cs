...
public class GameManager : MonoBehaviour {
    ...
    public void OnUserVariableUpdate(BaseEvent evt)
    {
        ...
        if (changedVars.Contains("x") && changedVars.Contains("y") && changedVars.Contains("z") && changedVars.Contains("rot"))
        {
            ...
            if (changedVars.Contains("targetDistance"))
                animator.SetFloat("TargetDistance", (float)user.GetVariable("targetDistance").GetDoubleValue());
            if (changedVars.Contains("Fire1"))
                animator.SetBool("Fire1", user.GetVariable("Fire1").GetBoolValue());
            if (changedVars.Contains("Dodge"))
                animator.SetBool("Dodge", user.GetVariable("Dodge").GetBoolValue());
            if (changedVars.Contains("Dive"))
	...
	private void SpawnLocalPlayer(int numModel, int numMaterial) {
		...
		localPlayerController = localPlayer.GetComponent<Player>();
        localPlayer.GetComponentInChildren<TextMesh>().text = sfs.MySelf.Name;
        Camera.main.gameObject.AddComponent<FollowCamera>();
        Camera.main.GetComponent<FollowCamera>().PlayerTransform = localPlayer.gameObject.transform;
	...
	private void SpawnRemotePlayer(SFSUser user, int numModel, int numMaterial, Vector3 pos, Quaternion rot, float moveVelocity) {
        ...
        animator.SetFloat("Vertical", moveVelocity);
